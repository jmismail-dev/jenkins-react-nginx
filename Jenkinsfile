        ...
        stage('Deploy') {
            steps {
                withCredentials([
                    ...
                    sh 'sudo rsync -avz --stats --rsync-path="echo ${SERVER_PASSWORD} | sudo -Sv && sudo rsync" ${WORKSPACE}/client/dist/ jmismail@${SERVER_IP_ADDRESS}:/var/www/jenkins-react-nginx/'
                }
            }
        }
    }
